...
export default function SignupPage() {
  ...
  const handleGoogleSignup = () => {
    // Redirect to backend Google OAuth endpoint (same as login for OAuth)
    const baseUrl = process.env.NEXT_PUBLIC_API_URL || `${window.location.protocol}//${window.location.hostname}:8080`;
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <Input
                ...
                required
                className={`auth-input h-12 text-base border-2 rounded-xl transition-all duration-300 group-hover:border-gray-300 dark:group-hover:border-gray-500 pl-12 pr-12 ${formData.confirmPassword && formData.password
                    ? passwordMatch
