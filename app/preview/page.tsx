...
export default function PreviewPage() {
  ...
  // Helper function to identify media fields
  ...
  const isVideoUrl = (value: unknown): boolean => {
    ...
    return /(\.mp4|\.webm|\.ogg|\.mov|\.avi|\.mkv)$/i.test(value.trim())
  }
    ...
    async function resolveHero() {
      ...
      if (!key) {
        key = keys.find((k) => {
          ...
          return (
            typeof v === 'string' &&
            /(\.png|\.jpg|\.jpeg|\.gif|\.webp)$/i.test(String(v))
          )
        })
        value = key ? obj[key] : undefined
      }
  ...
  useEffect(() => {
    async function resolveGridImages() {
      ...
      const resolvedMediaIds = new Set<number>()
      
      // First, resolve media IDs to URLs (images and videos)
      await Promise.all(
        Object.entries(obj)
          .filter(
            ([k, v]) =>
          ...
          .map(async ([k, v]) => {
            try {
              ...
              if (url) {
                ...
                resolvedMediaIds.add(Number(v))
              }
            } catch {}
          }),
      )
      
      // Then handle direct URL strings
      Object.entries(obj)
        .filter(
          ([k, v]) =>
            typeof v === 'string' &&
      ...
      // Handle direct video URL strings
      Object.entries(obj)
        .filter(
          ([k, v]) =>
            typeof v === 'string' &&
            /(\.mp4|\.webm|\.ogg|\.mov|\.avi|\.mkv)$/i.test(String(v)),
        )
        .forEach(([k, v]) => {
          const url = normalizeUrl(String(v))
    ...
    try {
      const pid =
        typeof window !== 'undefined'
          ? window.localStorage.getItem('active_project_id')
          : null
      if (pid) {
  ...
  const description = String(
    data.description || data.excerpt || data.summary || '',
  )
  ...
  const metaDesc =
    typeof data.meta_description === 'string'
      ...
      : undefined
  const canonical =
    typeof data.canonical_url === 'string' ? data.canonical_url : undefined
  
  // Separate content fields and media fields
  const contentFields = Object.entries(data).filter(([key]) => {
    ...
    return ![
      ...
      'thumbnail',
      'cover',
    ].some((k) => lower.includes(k)) && !isMediaField(key)
  })
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <button
                onClick={handleBack}
                className="p-2 -ml-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-600 dark:text-slate-400"
                aria-label="Go back"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div className="hidden sm:block h-6 w-px bg-slate-200 dark:bg-slate-700" />
              <h2 className="hidden sm:block text-sm font-medium text-slate-600 dark:text-slate-400 truncate max-w-xs lg:max-w-md">
                {title}
              </h2>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="relative h-[40vh] sm:h-[50vh] lg:h-[60vh] overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 lg:p-8">
              <div className="max-w-7xl mx-auto">
                ...
                {description && (
                  <p className="mt-3 text-base sm:text-lg text-slate-300 max-w-2xl line-clamp-2">
                    {description}
                  </p>
                )}
              </div>
          ...
          <div className="lg:col-span-2 space-y-4 lg:space-y-6">
            ...
            {description && (
              <section className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div className="p-4">
                  {renderTextContent(description, 'description')}
                </div>
              </section>
            )}
              ...
              <div className="p-4">
                {contentFields.length === 0 ? (
                  <p className="text-slate-500 dark:text-slate-400 text-center py-6 text-sm">
                    Tidak ada data konten tambahan
                  </p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {contentFields.map(([key, value]) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="group p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50"
                        >
                          ...
                          {isUrl ? (
                            <a
                              ...
                              className="text-sm text-blue-600 dark:text-blue-400 hover:underline break-all flex items-start gap-1.5"
                            >
                              {text}
                ...
                <div className="p-4">
                  <div className="grid grid-cols-1 gap-4">
                    {mediaFields.map(([key, value]) => {
                      ...
                      const isVideoUrl = typeof value === 'string' && /(\.mp4|\.webm|\.ogg|\.mov|\.avi|\.mkv)$/i.test(value.trim())
                      
                      // Skip rendering if this is a URL field and we already have the same media from media_id
                      if (typeof value === 'string' && (hasImage || hasVideo)) {
                        const isDuplicate = mediaFields.some(([otherKey, otherValue]) => {
                          return (
                            otherKey !== key &&
                            typeof otherValue === 'number' &&
                            (previewMap[otherKey] === hasImage || videoPreviewMap[otherKey] === hasVideo)
                          )
                        })
                      ...
                      return (
                        ...
                        >
                          ...
                          {hasImage ? (
                            ...
                            poster={
                              hasVideo
                                ? `${videoPreviewMap[key]}#t=0.1`
                                : heroSrc ||
                                  `data:image/svg+xml;base64,${btoa(
                                    `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#0f172a"/><stop offset="1" stop-color="#1e293b"/></linearGradient></defs><rect width="100%" height="100%" fill="url(#g)"/></svg>`
                                  )}`
                            }
                              >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
                    {entry.status}
                  </span>
                </div>
                <div className="flex items-center justify-between py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                ...
                {slug && (
                  <div className="flex items-center justify-between py-1.5">
                    ...
                    <span className="text-xs text-slate-900 dark:text-white font-mono truncate max-w-[140px]">
                      /{slug}
                    </span>
                  </div>
                )}
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm text-blue-600 dark:text-blue-400 font-medium truncate hover:underline cursor-pointer">
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2">
                      {metaDesc || 'Belum ada meta description'}
                    </p>
                  </div>
                </div>
