...
import { getInitials, getAvailableTransitions, isValidTransition, getStatusLabel } from "@/components/workflow-management/types";
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                          <Avatar className="w-8 h-8">
                            <AvatarFallback className="text-xs">
                              {(() => {
                                const user = creators[entry.id] ?? (entry as any).creator ?? undefined;
                          ...
                          <div className="flex flex-col">
                            <span
                              className="text-sm font-medium text-[var(--foreground)] truncate max-w-[120px]"
                              title={(() => {
                                const user = creators[entry.id] ?? (entry as any).creator ?? undefined;
                            ...
                            <span
                              className="text-xs text-[var(--muted-foreground)] truncate max-w-[120px]"
                              title={(() => {
                                const user = creators[entry.id] ?? (entry as any).creator ?? undefined;
                                return user?.email || "";
                              })()}
                      ...
                      <td className="py-3 px-4 text-[var(--muted-foreground)] whitespace-nowrap">
                        {new Date((entry as any).updated_at || Date.now()).toLocaleDateString()}
                      </td>
                      <td className="py-3 px-4">
                        {(() => {
