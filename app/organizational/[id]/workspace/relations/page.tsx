...
import { getStatusLabel, WorkflowStatus } from "@/components/workflow-management/types";
...
export default function OrgRelationsPage() {
  ...
  const canCreateRelation = ["projectadmin","projecteditor","projectcontentwriter"].includes(projectRoleKey);
  const canDeleteRelation = projectRoleKey === "projectadmin";
  ...
  const [entryMap, setEntryMap] = useState<Record<number, ContentEntry>>({});
  ...
  const titleOf = (e: ContentEntry | undefined, fallbackId: number) => {
    ...
    const load = async () => {
      try {
        if (fromEntryId && fromEntryId > 0) {
          const list = await relationsService.getRelations(fromEntryId, projectId);
          setRelations(list);
        } else {
          const { entries } = await searchService.fullText({ q: "", limit: 25, page: 1, project_id: projectId });
          const rels = await Promise.all(entries.map((e) => relationsService.getRelations(e.id, projectId)));
  ...
  useEffect(() => {
    ...
    let active = true;
    (async () => {
      const results = await Promise.allSettled(ids.map(id => contentService.getEntry(id, projectId)));
      if (!active) return;
      const map: Record<number, ContentEntry> = {};
      results.forEach((res, idx) => { if (res.status === "fulfilled") map[ids[idx]!] = res.value as ContentEntry; });
      setEntryMap(map);
    })();
    return () => { active = false; };
  }, [relations]);
  ...
  const filteredRelations = useMemo(() => {
    ...
    return relations.filter((relation) => {
      ...
      const fromStatus = fe ? getStatusLabel(fe.status as WorkflowStatus).toLowerCase() : "";
      const toStatus = te ? getStatusLabel(te.status as WorkflowStatus).toLowerCase() : "";
  ...
  const stats = getRelationStats(relations.map(r => ({ relation_type: r.relation_type as unknown as RelationType })));

  const handleView = (entryId: number) => {
    router.push(`/organizational/${projectId}/workspace/relations?from=${entryId}`);
  };

  const handleDelete = async (relationId: number) => {
    if (!confirm("Are you sure you want to delete this relation?")) return;
    if (!canDeleteRelation) { alert("no permission"); return; }
    try {
      await relationsService.deleteRelation(relationId, projectId);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)] transition-colors mt-1">
            Manage relationships within this project
          </p>
        </div>
        {canCreateRelation && (
          <Button
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {entryMap[relation.from_content_id] && (
                          <StatusBadge status={entryMap[relation.from_content_id].status as WorkflowStatus} />
                        )}
                      </div>
                    </div>
                  </td>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {entryMap[relation.to_content_id] && (
                          <StatusBadge status={entryMap[relation.to_content_id].status as WorkflowStatus} />
                        )}
                      </div>
                    </div>
                  </td>
      ...
      {showCreateModal && canCreateRelation && (
        <Modal
        ...
        >
          <RelationForm
            ...
            onSubmit={async (data) => {
              try {
                const created = await relationsService.createRelation(data.fromContentId, { to_content_id: data.toContentId, relation_type: data.relationType, project_id: projectId });
