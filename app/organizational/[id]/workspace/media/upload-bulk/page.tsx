...
export default function OrgBulkUploadMediaPage() {
  ...
  const handleUpload = (
    ...
    mediaService.bulkUpload(fs, metadata.folder, projectId).then(() => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-[var(--foreground)] transition-colors">Bulk Upload Media</h1>
          <p className="text-sm text-[var(--muted-foreground)] transition-colors mt-1">Upload multiple files at once</p>
        </div>
      </div>

      <div className="max-w-3xl">
        <MediaUploadForm folders={folders} onUpload={handleUpload} onCancel={handleCancel} mode="bulk" />
      </div>
    </div>
  );
}
